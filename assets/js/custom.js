...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function persianYear(timestamp) {
    const stampDate = new Date(timestamp * 1000);
    const faDay = stampDate.toLocaleDateString('fa-IR', { year: 'numeric' });
    return faDay;
  }
  ...
  document.querySelectorAll('.persianYear').forEach(function (el) {
    const timestamp = el.dataset.timestamp;

    if (timestamp) {
      el.textContent = persianYear(Number(timestamp));
    }
  });
